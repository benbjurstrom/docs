...
## Using Objects as Responses

Data objects implement `Arrayable` and `JsonSerializable`, so Laravel automatically converts them to JSON when returned from routes and controllers.

### Returning Objects

Return a data object directly and Laravel handles the conversion:

```php icon=php
public function show(User $user): UserObject
{
    return $user->toObject();
}
```
...
For custom status codes or headers, use Laravel's `response` helper:

```php icon=php
return response()->json($user->toObject(), 201)
    ->header('X-User-Id', $user->id);
```

### Paginated Responses

Laravel's paginator works seamlessly with data objects. Use the `through` method to transform each model:

```php icon=php
return User::paginate()->through(fn ($user) => $user->toObject());
```
